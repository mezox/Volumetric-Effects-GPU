...
namespace vfx
{
	void TempInjection::inject(Quantity* quantity, const glm::vec3& position, float deltaTime)
	{
		auto pipeline = system::Renderer::getInstance().getPipelineByName("injection1D");
		pipeline->Bind();
